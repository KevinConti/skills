...
| 2. Draft brief — shape sub-agent | delegated | Draft the design brief + open design questions (impeccable's judgment) | [tasks/shape.md](tasks/shape.md) |
| 3. Resolve design + commit contract — `interactive-plan` | inline ⛔ | Ask the open design questions, then the contract; keep the brief file current | [tasks/contract.md](tasks/contract.md) |
...
This preserves every gate exactly — the user still sees the same diagrams, screenshots, and findings — while keeping your window lean. **Inline steps stay inline for good reasons:** `interactive-plan` is live human collaboration — the user answers in its UI, which only the top-level session receives (this is also why the design questions route through it, rather than impeccable's separate chat interview); `handoff` is cheap because you already hold the plan context and only need the doc's path; the gates and handback *are* your job.
